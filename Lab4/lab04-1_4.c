...
int main(int argc, char *argv[])
{
    ...
    double wave_length; // identifier corrected - replaced with _
    double length_in_meters;
    double plankconst;
    double energy; //identifier cannot begin with a number
    // all identifiers declared incorrectly; corrected

    plankconst = 6.62606957 * pow(10, -34); // Planck's constant
    speed_of_light = 2.99792458 * pow(10, 8); // Constant for the speed of light
    wave_length = 0;
    length_in_meters = 0;
    energy = 0; //identifier corrected
    ...
    scanf("%lf", &wave_length); //identifier corrected

    if (wave_length > 0.0) //identifier corrected
    {
        length_in_meters = wave_length / pow(10, 9); // Converting nano-meters to meters //identifier corrected
        energy = (plankconst * speed_of_light) / length_in_meters; // identifier corrected// Calculating the energy of 1 photon
        printf("A photon with a wave-length of %08.3lf nano-meters, carries "
               "\napproximately %030.25lf joules of energy.", wave_length, energy); //identifier wave_length and energy corrected
    } else
    {
