...
int main(int argc, char *argv[])
{
    ...
    if (number > 10000 || number < -10000) // | changed to ||
    {
        printf("Number is out of range!\n");
        return -1;
    }

    if ((is_positive(number) || (is_negative(number))) || is_zero(number)) //& changed to && and | changed to ||, removed !
    {
...
int is_zero(int number)
{
    ...
    {
        //printf("%d is zero and ", number); corrected code below
